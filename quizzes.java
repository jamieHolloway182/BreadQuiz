...
public class quizzes extends Frame {
    ...
    int definitionHovered;
    String[] definitions = {"Inclusionary: belief that the slices of a sandwich may be made of a liberal range of items",
    		"Structuralist: belief that the structure of a bread's slices must closely resemble that of the classic sandwich",
    		"Fundamentalist: belief that the ingredients in the sandwich's slices must strictly resemble those in the classic sandwich",
    		"Abstractionist: belief that the structure of a bread's slices may have a large and liberal range of forms"};
    ...
    public void paint(Graphics g) {
        if (questionsDone < allQs.getTotalQuestions()) {
            ...
            finished = true;
        	if (!(definitionHovered == -1)) {
        		g.drawString(definitions[definitionHovered],100,50);
        	}
        }

    }

    public void giveDefs() {
        int x = m.getX();
        int y = m.getY();
        int hold = definitionHovered;
        if(Math.abs(FrameHeight/2 - y) < 15) {
	        if(x > FrameWidth/2 - axisSize/2 -85 && x < FrameWidth/2 - axisSize/2) {
	            definitionHovered = 0;
	        }else if (x > FrameWidth/2 + axisSize/2 && x < FrameWidth/2 + axisSize/2 + 110) {
	        	definitionHovered = 2;
	        }else {
	        	definitionHovered = -1;
	        }
        }else if(Math.abs(FrameWidth/2 - x) < 50){
        	if(y > FrameHeight/2 - axisSize/2 -17 && y < FrameHeight/2 - axisSize/2) {
        		definitionHovered = 1;
        	}else if(y > FrameHeight/2 + axisSize/2 && y < FrameHeight/2 + axisSize/2+15) {
        		definitionHovered = 3;
        	}else {
        		definitionHovered = -1;
        	}
